...
- **Todos en la periferia, y al alcance de la mano.** La cátedra pidió que los objetos no coincidan con la zona central, donde está la persona. `CONFIG.fondo.zonaDeLaPersona` (cabeza y hombros, normalizados al espejo) y el pie del nombre son zonas prohibidas: `tests/integracion/fondos.test.js` lo vigila para cada fondo del catálogo, el respaldo vectorial y los lugares por defecto incluidos, y también que los cuatro objetos de un fondo no se pisen y que **la mano llegue a los cuatro**: alguien sentado a 2 m, con los hombros de la prueba del sostenido y el brazo de `tablero.radioFactor` desde cada hombro. La periferia tira para arriba y el brazo no; es un modelo, y en el stand se prueba con gente de verdad. Los lugares salieron de medir cada foto —dos rincones por costado, uno alto con su ficha por encima y uno a la altura de la cara con su ficha por debajo, el más oscuro de cada uno— y se miran en `herramientas/fondos.html`, que tiene una casilla para dibujar la zona.
- **Se mecen apenas, cada uno a su ritmo** (`balanceo`, `CONFIG.escondidos.balanceo`): es el "pequeño movimiento para que la persona los pueda identificar". Otro período y otra fase por objeto —tres al unísono se leen como una animación pegada encima— y siempre continuo: a los saltos se lee como un parpadeo. Cómo se ve cada uno en cada cuadro —el vaivén o la flotación, y cuánto crece, se calma y se ilumina el que se está leyendo— lo dice **`aspectoDelObjeto`, que usan el espejo y `herramientas/fondos.html`**: la herramienta existe para mostrar exactamente lo que hace el espejo, y con la cuenta copiada en los dos, ajustar uno solo los separaba en silencio. **El que se está leyendo va además delante de la persona** —el mismo objeto, sin halo, con el alfa de su ficha—: detrás, la mano que lo fue a buscar lo tapaba justo cuando se ilumina.
...
- **La ficha no le tapa la cara a nadie.** `disponerFicha` (escena.js) la pone en la franja del costado de su objeto (`fichas.columna`, el mismo borde que la cabeza en la zona de la persona): por eso es angosta y alta y no un cartel al lado del objeto, que caería encima de la cara. Va debajo del objeto si está arriba y arriba si está abajo, nunca encima del que describe, del otro lado si taparía a otro objeto del fondo (`evitar`) y nunca en el pie. El nombre se mide —si no entra va en dos renglones, y si una palabra sola no entra se achica— y la letra es `fichas.tipografia`. `tests/integracion/fichas.test.js` dispone las fichas de todo el catálogo real y exige que entren enteras, en su franja, sin tapar a su objeto ni a los otros: así apareció que "Lector de código de barras" se salía de la pantalla y que una de cada seis fichas tapaba a un vecino.
