...
**El objeto aterriza siempre a la vista, y eso no lo garantiza el `lugar` solo.** Las fotos se preparan en 1080×1920 para el espejo vertical; en un monitor apaisado (desarrollo) la foto entra al ancho y sólo se ve su franja del medio, y los lugares —todos en el tercio de arriba, que es donde está el rincón oscuro— quedan recortados: el objeto volaba arriba del borde y desaparecía en las doce ingenierías. Por eso `lugarEnPantalla` recibe también la pantalla y, si el lugar cae fuera, lo corre lo justo para que el objeto entre entero (`fondo.margenDelLugar`, en radios). En el espejo vertical no mueve nada; `tests/integracion/fondos.test.js` lo fija con el catálogo real en las dos orientaciones.
