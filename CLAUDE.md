...
| `npm run listo` | Semáforo de contenido: valida la jerarquía de carpetas en `contenido/carreras/`, los **cuatro objetos** de cada carrera con su `nombre` y una `descripcion` de hasta 130 caracteres, el fondo activo con sus coordenadas, el video de humo en `contenido/comun/`, la tipografía Muffaroo con su nota de licencia, que cada `maite` exista del otro lado y MediaPipe vendorizado. **Hoy está en verde.** Corre `tests/listo/`, excluido de `npm test` a propósito: depende de archivos que pueden faltar en una máquina de desarrollo. |
| `npm start` | Servidor en :8080. Genera `contenido/catalogo.json` si falta. El espejo se abre por `http://localhost:8080/espejo/espejo.html` — **nunca por IP ni por `file://`**: Chrome solo entrega la cámara en contextos seguros, y esa es la única razón por la que hay un servidor. |
| `npm run vendorizar` | Copia MediaPipe a `vendor/` y baja los modelos. Único paso que necesita red; se corre una sola vez. |
| `npm run catalogo` | Descubre la estructura de carpetas `contenido/carreras/`, valida los metadatos y genera `contenido/catalogo.json`. |
...
Todas las carreras jugables se ofrecen (`opciones`), en orden barajado por sesión, al entrar a HUMO. Los objetos representativos del carrusel se cargan antes de iniciar cámara y detectores; el fondo y los objetos escondidos de la carrera elegida se piden en segundo plano al recibir `mira`. `carrera` queda en null hasta que la persona agarra algo.
...
**`tiempos.eleccionMaxima` (30 s) es la red de seguridad de la fila**: si nadie agarró nada, cierra sin asignar una carrera ni avisar a MAITE. A los 10 s, `tiempos.ayudaEleccion` refuerza la consigna una vez. **La red no se le cae encima a quien ya está eligiendo**: con un sostenido en curso (`eligiendo`, que main.js le pasa a `actualizar`) el plazo espera hasta que el gesto se complete o se suelte. Tras el cierre, el mismo rostro no inicia otra sesión; la máquina se rearma cuando observa una ausencia.
...
- `vision.js` carga el WASM de MediaPipe una sola vez para los tres detectores (rostro, manos, pose). Manos y pose son agregados opcionales: si pose falla, el espejo conserva el fondo tenue; si manos no carga, la consigna habilita el puntero o táctil como alternativa. `npm run listo` verifica igualmente que los tres `.task` estén.
...
- **Todos en la periferia, y al alcance de la mano.** La cátedra pidió que los objetos no coincidan con la zona central, donde está la persona. `CONFIG.fondo.zonaDeLaPersona` (cabeza y hombros, normalizados al espejo) y el pie del nombre son zonas prohibidas: `tests/integracion/fondos.test.js` lo vigila para cada fondo del catálogo, el respaldo vectorial y los lugares por defecto incluidos, y también que los cuatro objetos de un fondo no se pisen y que **la mano llegue a los cuatro**, con un 10 % de margen: la misma persona que la zona —hombros donde empieza el cuerpo, 380 px de ancho como la de la prueba del sostenido— y el brazo del carrusel, `tablero.radioFactor` anchos de hombros desde el centro de los hombros. La periferia tira para arriba y el brazo no; es un modelo, y si en el stand la gente queda más abajo en el cuadro, a los de arriba no llega: se recalibran juntas la zona y los lugares. Todos los fondos usan **la misma grilla** —dos columnas pegadas a la zona de la cabeza y dos filas, una justo debajo del cartel de las fichas y otra a la altura de los hombros, con `escala` 0.24, que deja aire entre los dos de cada costado para que se note cuál tiene la mano encima—; de una foto a otra cambia qué objeto va en cada rincón. La prueba vigila además que **los blancos de la mano de dos objetos no se toquen** (`fichas.radioFactor`): tocándose, yendo a buscar el de abajo se abría el de arriba. Se miran en `herramientas/fondos.html`, que tiene una casilla para dibujar la zona.
- **Se mecen apenas, cada uno a su ritmo** (`balanceo`, `CONFIG.escondidos.balanceo`): es el "pequeño movimiento para que la persona los pueda identificar". Otro período y otra fase por objeto —tres al unísono se leen como una animación pegada encima— y siempre continuo: a los saltos se lee como un parpadeo. Cómo se ve cada uno en cada cuadro —el vaivén o la flotación, y cuánto crece, se calma y se ilumina el que se está leyendo— lo dice **`aspectoDelObjeto`, que usan el espejo y `herramientas/fondos.html`**: la herramienta existe para mostrar exactamente lo que hace el espejo, y con la cuenta copiada en los dos, ajustar uno solo los separaba en silencio. **El que tiene la mano encima pasa además delante de la persona**, recortado contra su silueta (`dibujarObjetosDelante`), y crece y se ilumina, apenas la mano lo toca y sin esperar a su ficha: `delante` en `fichas.js`, que entra en `fichas.msDelante` (150 ms) y se sostiene la misma gracia que la ficha. Detrás, la mano que lo iba a buscar lo tapaba; cuando pasaba adelante recién con la ficha abierta, casi un segundo después, la mano parecía atravesarlo. Aparece sólo donde la persona lo tapa; dibujado entero encima de sí mismo, duplicaba la sombra y los bordes del PNG.
- **La ficha se abre al pasar la mano**, sobre cualquiera de los cuatro. `fichas.js` es el hermano tranquilo del sostenido: abrir pide `fichas.msParaMostrar` con la mano encima —si no, cada mano que pasa camino a otro lado abre fichas en cadena—, cerrar pide `fichas.msDeGracia` sin ella —absorbe los huecos de la detección y deja terminar de leer después de bajar la mano—, y cada ficha tiene su alfa: pasar de un objeto a otro es un fundido cruzado, nunca un corte. Se lee sobre los objetos **quietos**, no sobre su vaivén: si el blanco se meciera con el objeto, la ficha se abriría y cerraría sola con la mano quieta en el borde. Cada objeto se identifica por su lugar en `objetos` (con la ruta del PNG, dos con la misma imagen compartirían la ficha), y los escondidos se leen recién cuando se ven a medias (`fichas.alfaParaLeer`).
- **La ficha va arriba de la cabeza, en un cartel ancho.** `disponerFicha` (escena.js) la pone en la franja de arriba de la zona de la persona —de `zonaDeLaPersona[0].y0` para arriba, a lo ancho de la composición—, la única que no le tapa la cara a nadie y donde la descripción entra en dos o tres renglones con letra grande (`fichas.tipografia`). Antes iba al costado de su objeto, en la franja angosta de la periferia, y con objetos grandes y letra que se lea a dos metros no entraba: dos objetos y sus dos fichas por costado no cabían, y la ficha de uno tapaba al otro. De cuál habla lo dice el objeto, que crece y se ilumina mientras se lee. Si una descripción no entrara, la letra se achica sola antes que bajar hasta la cara; `tests/integracion/fichas.test.js` dispone las fichas de todo el catálogo real y exige que entren enteras en la franja **con la letra de la config**, sin tapar a ningún objeto del fondo, en el espejo y en apaisado. El nombre se mide: si no entra va en dos renglones, y si una palabra sola no entra se achica. Hasta dónde baja y con qué letra lo dice `fichaDelObjeto`, la misma en el espejo, la herramienta y las pruebas.
...
su nota de licencia viven en `contenido/comun/tipografias/`, y `npm run listo`
...
- **La fuente de verdad del contenido es la estructura física de carpetas en `contenido/carreras/<id>/`:** `carrera.json` (nombre, color, maite, fondo activo opcional), `objetos/<id>/` (`imagen.png`, `metadata.json`) y `fondos/<id>/` (`imagen.jpg`, `video.mp4` opcional, `metadata.json`). Se genera automáticamente `contenido/catalogo.json` en disco (ignorado en git). Agregar o cambiar una carrera no toca código, pero sí necesita su tecla en `operacion.js`, vigilada por `tests/integracion/atajos.test.js`. Orden de dibujo de un objeto: PNG → figura vectorial (`espejo/figuras.js`) → círculo del dorado de la paleta. Fondo: video (`espejo/videos.js`) → PNG/JPG → escena vectorial (`espejo/escenarios.js`) → color plano de la carrera.
- **Cada sesión elige un objeto al azar para el carrusel** de entre los cuatro de la ingeniería, y distribuye los otros tres al azar en los escondites del fondo. No hay orden semántico ni objeto principal fijo.
