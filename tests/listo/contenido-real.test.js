...
const obtenerCatalogo = async () => {
  const { catalogo, errores } = await construirCatalogo({
    raizCarreras: resolve(CONTENIDO, 'carreras'),
    figurasValidas: figurasDisponibles(),
  });
  if (errores.length > 0) throw new Error(`Errores de contenido:\n  - ${errores.join('\n  - ')}`);
  return catalogo;
};
...
describe('contenido real', () => {
  it('pasa la validacion del sistema', async () => {
    const crudas = await descubrirEstructura({ raizCarreras: resolve(CONTENIDO, 'carreras') });
    const errores = validarEstructura(crudas, { figurasValidas: figurasDisponibles() });
  ...
  it('cada fondo dice donde se apoya el objeto', async () => {
    ...
    for (const carrera of (await obtenerCatalogo()).carreras) {
      for (const fondo of carrera.fondos ?? []) {
        if (!fondo.lugar) sinLugar.push(fondo.img);
      }
    }
  ...
  it('el video de humo esta copiado al contenido', async () => {
    expect(await existe('comun/humo.mp4'), 'falta contenido/comun/humo.mp4').toBe(true);
  });
  ...
  it('la tipografia de las tablets de MAITE esta copiada, con su nota de licencia', async () => {
    for (const archivo of [
      'comun/tipografias/Muffaroo-Regular.ttf',
      'comun/tipografias/Muffaroo-LEEME.txt',
    ]) {
  ...
  it('cada "maite" declarado existe del otro lado', async () => {
    ...
    const huerfanas = (await obtenerCatalogo()).carreras
  ...
  it('hay al menos una carrera jugable', async () => {
    const jugables = (await obtenerCatalogo()).carreras.filter((c) => c.maite).map((c) => c.id);
  ...
  it('hay carreras jugables suficientes para que el carrusel sea un carrusel', async () => {
    const jugables = (await obtenerCatalogo()).carreras.filter((c) => c.maite);
